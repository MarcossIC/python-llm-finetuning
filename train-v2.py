...
def optimize_hyperparams(args, using_gpu, logger):
    """
...
def get_model_targets(model_config, logger):
    ...
    elif 'neox' in arch or "gpt-neox" in arch:
...
def load_model(args, logger, using_gpu):
    """
    Carga y prepara un modelo para CPU o GPU con manejo robusto de errores.
    """
    ...
    try:
        ...
        if tokenizer.pad_token is None:
            if tokenizer.eos_token is not None:
                ...
                logger.warning("⚠️ Tokenizer no tenía pad_token, se ha establecido pad_token = eos_token.")
            else:
                tokenizer.add_special_tokens({'pad_token': '[PAD]'})
                logger.warning("⚠️ Tokenizer no tenía pad_token ni eos_token; se ha añadido un nuevo token [PAD].")
    ...
    try:        
        if using_gpu:
            ...
            gpu_mem = max(1, total_gpu - 1)  # Dejar 1GB libre
            avail_cpu = psutil.virtual_memory().available / (1024**3)
            cpu_mem = max(1, avail_cpu - 2)  # Dejar 2GB libre
            ...
            for i, strategy in enumerate(load_strategies):
                try:
                    logger.info(f"🔄 Trying model loading strategy {i+1}/{len(load_strategies)}...")
                    model = strategy()
        ...
        # Configurar LoRA (común para CPU y GPU)
...
def _load_gpu_model(args, logger, offload=False, fp32_cpu_offload=False, force_max_quantization=False):
    ...
    bit_precision = 4 if force_max_quantization else args.bit_precision
    quant_cfg = None
    ...
    gpu_mem = max(1, torch.cuda.get_device_properties(0).total_memory / (1024**3) - 1)
    cpu_mem = max(1, psutil.virtual_memory().available / (1024**3) - 2)
    
    # Configurar memory map según offload
    if offload:
        mem_map = {0: f"{int(gpu_mem)}GiB", 'cpu': f"{int(cpu_mem)}GiB"}
        logger.info(f"🔄 Loading with CPU offload. Memory budget: GPU={int(gpu_mem)}GiB, CPU={int(cpu_mem)}GiB")
    else:
        mem_map = {0: f"{int(gpu_mem)}GiB"}
        logger.info(f"🔄 Loading GPU-only. Memory budget: GPU={int(gpu_mem)}GiB")
    
    # Device map automático solo para modelos grandes
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        max_memory=mem_map,
        offload_folder='offload' if offload else None,
...
def process_dataset(args, tokenizer, logger, using_gpu):
    """
    ...
    try:
        ...
        if using_gpu and len(raw) > 10000:
            ...
            formatted = raw.map(
                format_example,
                remove_columns=raw.column_names,
                num_proc=1 if not using_gpu else min(os.cpu_count() or 1, 4),
                desc="Formatting dataset"
            )
...
def main():
    ...
    logger.info(f"🖥️ Using {'GPU' if using_gpu else 'CPU'} for training")
    # 5) Optimizar hiperparámetros
    args = optimize_hyperparams(args, using_gpu, logger)
    ...
    model, tokenizer = load_model(args, logger, using_gpu)
    ...
    train_ds, eval_ds = process_dataset(args, tokenizer, logger, using_gpu)
